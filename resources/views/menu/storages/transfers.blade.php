            ...
            <div class="layout__content">
                ...
                <div class="center-content">
                    <div class="content__inputs">
                        ...
                        <a href="{{ url('storages/create') }}">
                            <input type="submit" class="inputs__buttom" value="CREAR TRASLADO">
                        </a>
                    </div>
                    ...
                    <div class="content__data">
                        <table class="products__table">
                            <thead class="table__th">
                                <tr>
                                    <th class="th__title">Id Traslado</th>
                                    <th class="th__title">Fecha de Traslado</th>
                                    <th class="th__title">Almacen</th>
                                    <th class="th__title">Producto</th>
                                    <th class="th__title">Cantidad</th>
                                    <th class="th__title">Destino</th>
                                    <th class="th__title">Estado</th>
                                    <th class="th__title">Observaciones</th>
                                    <th class="th__title"></th>
                                </tr>
                            </thead>
                            <!-- <tbody class="table__td">
                                @foreach($transfers as $transfer)
                                <tr>
                                    <td class="td__data supplier-id">{{ $transfer->store_id }}</td>
                                    <td class="td__data supplier-name">{{ $transfer->name_store }}</td>
                                    <td class="td__data supplier-email">{{ $transfer->address_store }}</td>
                                    <td class="td__data supplier-city">{{ $transfer->incharge_store }}</td>
                                    <td class="td__data supplier-phone">{{ $transfer->phone }}</td>
                                    <td class="td__data supplier-buttoms">
                                        <a href="{{ url('/storages/'.$transfer->store_id.'/edit') }}">
                                            <i class="fa-solid fa-pen-to-square"></i>
                                        </a>
                                        <form action="{{ url('/storages/'.$transfer->store_id) }}" class="d-inline" method="post">
                                            @csrf
                                            {{ method_field('DELETE') }}

                                            <i class="delete__icon fa-solid fa-trash-can" onclick="return confirm('¿Deseas eliminar este almacen?')" value="Borrar"></i>
                                        </form>
                                    </td>
                                </tr>
                                @endforeach
